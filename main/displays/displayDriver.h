...
// NOTE: yysluping "big screen" NerdQAxe++ variant uses a 3.5" 480x320 ST7796 panel
// on the same 8-bit parallel (i80) bus as the stock 1.9" 320x170 T-Display S3.
// ST7796 is command-compatible with the st7789 init used below. Landscape via swap_xy.
#define TDISPLAYS3_LCD_H_RES 480                                            // Horizontal resolution (big-screen variant)
#define TDISPLAYS3_LCD_V_RES 320                                            // Vertical resolution (big-screen variant)
...
class DisplayDriver {
  ...
  protected:
    ...
    // cumulative "total mined" runtime (persisted to NVS), shown on the sigil screen
    uint64_t m_totalRuntimeBase = 0;     // seconds accumulated before this session
    int64_t  m_lastRuntimePersist_us = 0;
    bool     m_runtimeLoaded = false;

    // lifetime accepted shares (persisted to NVS). Accumulated by delta so pool
    // reconnects (which reset the session counter) don't lose shares.
    uint64_t m_lifetimeAccepted = 0;
    uint64_t m_lastSessionAccepted = 0;
    int64_t  m_lastAcceptPersist_us = 0;
    bool     m_acceptedLoaded = false;
    ...
    void fadeInMiningStats();                                       // fade in stat labels (boot, solid bg)
