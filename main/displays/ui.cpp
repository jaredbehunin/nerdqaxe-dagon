...
void UI::splash2ScreenInit(void)
{
    ...
    lv_obj_set_align(ui_lbConnect, LV_ALIGN_BOTTOM_MID);
    lv_obj_set_pos(ui_lbConnect, 0, -16);
    lv_label_set_text(ui_lbConnect, "SUMMONING THE SIGIL...");
    lv_obj_set_style_text_color(ui_lbConnect, lv_color_hex(0xFFC45C), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_lbConnect, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(ui_lbConnect, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void UI::miningScreenInit(void)
{
    ...
    // --- Mehrunes Dagon Sigil screen: animated centerpiece + edge stats --------
    // The mining background is the static sigil altar; an lv_animimg loops the
    // flowing-lava frames over the central column. Live stats sit in the 4 corners
    // so the sigil stays clear. Colors are intended values (pipeline fixed via
    // LV_COLOR_16_SWAP + BGR). Applied last so it wins over creation-time layout.
    {
        extern const void *sigil_anim_frames[];
        extern const int sigil_anim_frame_count;
        extern const void *sigil_refl_frames[];
        extern const int sigil_refl_frame_count;

        const lv_color_t GOLD   = lv_color_hex(0xFFC45C); // captions
        const lv_color_t MOLTEN = lv_color_hex(0xFF7A1A); // values
        const lv_color_t ASH    = lv_color_hex(0xA58276); // footer

        // animated lava column over the altar (crop box was 150,58 - 330,286)
        lv_obj_t *sigil = lv_animimg_create(ui_MiningScreen);
        lv_obj_set_align(sigil, LV_ALIGN_TOP_LEFT);
        lv_obj_set_pos(sigil, 150, 58);
        lv_animimg_set_src(sigil, (const void**)sigil_anim_frames, sigil_anim_frame_count);
        lv_animimg_set_duration(sigil, 800);
        lv_animimg_set_repeat_count(sigil, LV_ANIM_REPEAT_INFINITE);
        lv_animimg_start(sigil);

        // reflection on the wet floor below the altar: same flicker flowing downward
        lv_obj_t *refl = lv_animimg_create(ui_MiningScreen);
        lv_obj_set_align(refl, LV_ALIGN_TOP_LEFT);
        lv_obj_set_pos(refl, 120, 280);
        lv_animimg_set_src(refl, (const void**)sigil_refl_frames, sigil_refl_frame_count);
        lv_animimg_set_duration(refl, 800);
        lv_animimg_set_repeat_count(refl, LV_ANIM_REPEAT_INFINITE);
        lv_animimg_start(refl);

        // helper: a small caption label
        auto caption = [&](const char *txt, lv_align_t al, int x, int y) {
            lv_obj_t *c = lv_label_create(ui_MiningScreen);
            lv_label_set_text(c, txt);
            lv_obj_set_style_text_color(c, GOLD, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_text_font(c, &ui_font_OpenSansBold13, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_align(c, al);
            lv_obj_set_pos(c, x, y);
            return c;
        };
        auto value = [&](lv_obj_t *lbl, lv_align_t al, int x, int y, lv_text_align_t ta) {
            lv_obj_set_align(lbl, al);
            lv_obj_set_pos(lbl, x, y);
            lv_obj_set_style_text_align(lbl, ta, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_text_color(lbl, MOLTEN, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_text_font(lbl, &ui_font_OpenSansBold24, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_move_foreground(lbl);
        };

        // corners: HASHRATE (TL) TEMP (TR) BEST (BL) EFF (BR)
        caption("HASHRATE", LV_ALIGN_TOP_LEFT, 10, 8);
        value(ui_lbHashrate, LV_ALIGN_TOP_LEFT, 10, 22, LV_TEXT_ALIGN_LEFT);
        caption("TEMP", LV_ALIGN_TOP_RIGHT, -10, 8);
        value(ui_lbTemp, LV_ALIGN_TOP_RIGHT, -10, 22, LV_TEXT_ALIGN_RIGHT);
        // bottom corners: caption sits well above the value so they don't collide
        caption("BEST", LV_ALIGN_BOTTOM_LEFT, 10, -32);
        value(ui_lbBestDifficulty, LV_ALIGN_BOTTOM_LEFT, 10, -6, LV_TEXT_ALIGN_LEFT);
        lv_obj_clear_flag(ui_lbEficiency, LV_OBJ_FLAG_HIDDEN);
        caption("EFF", LV_ALIGN_BOTTOM_RIGHT, -10, -32);
        value(ui_lbEficiency, LV_ALIGN_BOTTOM_RIGHT, -10, -6, LV_TEXT_ALIGN_RIGHT);

        // featured: TIME IN OBLIVION counter (X years, Y days, Z hours) bottom-center.
        // ui_lbTime is repurposed by updateTime(); use the small caption font so the
        // spelled-out value reads cleanly, just below the title.
        lv_obj_clear_flag(ui_lbTime, LV_OBJ_FLAG_HIDDEN);
        lv_obj_t *minedCap = caption("TIME IN OBLIVION", LV_ALIGN_BOTTOM_MID, 0, -33);
        lv_obj_set_style_text_align(minedCap, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_align(ui_lbTime, LV_ALIGN_BOTTOM_MID);
        lv_obj_set_pos(ui_lbTime, 0, -14);
        lv_obj_set_style_text_align(ui_lbTime, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_text_color(ui_lbTime, GOLD, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_text_font(ui_lbTime, &ui_font_OpenSansBold13, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_move_foreground(ui_lbTime);

        // plain digital side groups (size = title font, neutral color, one line/row,
        // each group vertically centered). Values updated live in updateGlobalState().
        const lv_color_t INFO = lv_color_hex(0x959595);  // medium gray, unthemed
        auto info = [&](lv_obj_t **slot, lv_align_t al, int x, int y, lv_text_align_t ta) {
            lv_obj_t *l = lv_label_create(ui_MiningScreen);
            *slot = l;
            lv_obj_set_style_text_color(l, INFO, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_text_font(l, &ui_font_OpenSansBold13, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_text_align(l, ta, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_align(l, al);
            lv_obj_set_pos(l, x, y);
            lv_label_set_text(l, "");
            lv_obj_move_foreground(l);
        };
        // left group: power / freq / vcore (tighter line spacing)
        info(&ui_lbSigPower, LV_ALIGN_LEFT_MID, 10, -22, LV_TEXT_ALIGN_LEFT);
        info(&ui_lbSigFreq,  LV_ALIGN_LEFT_MID, 10,   0, LV_TEXT_ALIGN_LEFT);
        info(&ui_lbSigVcore, LV_ALIGN_LEFT_MID, 10,  22, LV_TEXT_ALIGN_LEFT);
        // right group: accepted / rejected / blocks
        info(&ui_lbSigAccepted, LV_ALIGN_RIGHT_MID, -10, -22, LV_TEXT_ALIGN_RIGHT);
        info(&ui_lbSigRejected, LV_ALIGN_RIGHT_MID, -10,   0, LV_TEXT_ALIGN_RIGHT);
        info(&ui_lbSigBlocks,   LV_ALIGN_RIGHT_MID, -10,  22, LV_TEXT_ALIGN_RIGHT);

        // hide the rest to keep the sigil clear
        for (lv_obj_t *h : { ui_lbVcore, ui_lbIntensidad, ui_lbPower, ui_lbRPM,
                             ui_lbIP, ui_lbVinput, ui_lbASIC }) {
